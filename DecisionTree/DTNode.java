...
public class DTNode{
    ...
    private DTNode yes;
    private DTNode no;
    private DTNode other;
    private boolean moreNodesPresent;
    
    private List<DTNode> moreNodes = new ArrayList<DTNode>(); // list of more nodes if the user wishes to add them
    //private Map<String, DTNode> moreNodes = new HashMap<String, DTNode>(); // list of more nodes if the user wishes to add them
    ...
    /**
     * Construct a new node with text and one child
     */
    public DTNode(String txt, DTNode child){
        text = txt;
        other = child;
        moreNodesPresent = false;
    }
    ...
    public DTNode getOther(){return other;}
    ...
    /**
     * Sets a node with multiple children node
     */
    public void setMoreNode(DTNode newChild){
        moreNodesPresent = true;
        moreNodes.add(newChild);
    }
    
    public boolean getMoreNodes(){
        return moreNodesPresent;
    }
    
    public List<DTNode> getList(){
        return moreNodes;
    }
    
    public List<String> getStringList(){
        List<String> text = new ArrayList<String>();
        for(DTNode node: moreNodes){
    ...
    public int getChildrenCount(){
        ...
        else if(moreNodes.size() == 0){
            return 2;
        }
        else{
            return moreNodes.size();
        }
    }
